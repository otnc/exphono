...
ExpHono is verified against the real Express test suite, not just its own tests. As of the last recorded run, it passes 1094 of 1116 Express 5 tests (about 98%); the failure count is tracked as a ratchet so it can only improve. See [test/express-suite/](./test/express-suite/) for how the suite is run.
