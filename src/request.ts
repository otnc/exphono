...
function headerOf(req: ExpRequest, name: string): string | string[] | undefined {
  ...
  if (Object.hasOwn(req, 'headers')) {
    const materialized = req.headers as Record<string, string | string[] | undefined>
    if (lower === 'referer' || lower === 'referrer')
      return materialized.referrer ?? materialized.referer
    return materialized[lower]
  }

  const headers = req[kState].ctx.req.raw.headers
  if (lower === 'referer' || lower === 'referrer') {
    return headers.get('referrer') ?? headers.get('referer') ?? undefined
  }
  if (lower === 'set-cookie') {
